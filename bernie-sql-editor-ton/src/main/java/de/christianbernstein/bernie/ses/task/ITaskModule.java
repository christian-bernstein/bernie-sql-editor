 ...
 */
public interface ITaskModule extends IBaseModuleClass<ITon> {

    @ModuleDefinition(into = Constants.tonEngineID)
    Module<ITon> taskModule = Module.<ITon>builder()
            .name("task_module")
            .build()
            .$(module -> module.getShardManager().install(TaskModule.class));

    TaskStruct getStruct(String id);
